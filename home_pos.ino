void home_majumundur() {
  ...
  for (int i = 0; i < 3; i++ ) {
    ...
    IK_Kanan1(tmp_pos);
//    IK_Kiri1(tmp_pos);
    pos_homecoxamajumundur = angle_kanan[i];
    pos_homefemurmajumundur = angle_kanan[i];
    pos_hometibiamajumundur = angle_kanan[i];
  }
}

void goto_servo_home_majumundur(float servo_c, float servo_f, float servo_t){
  ...
  realsudutrc2 = 96 - servo_c;
  pwmrc2 = 2.78 * (realsudutrc2 + 36);

  realsudutrf2 = (90 - servo_f) / 1.022 ;
...
void home_belok() {
  ...
  for (int i = 0; i < 3; i++ ) {
    ...
    IK_Kanan1(tmp_pos);
//    IK_Kiri1(tmp_pos);
    pos_homecoxabelok = angle_kanan[i];
    pos_homefemurbelok = angle_kanan[i];
    pos_hometibiabelok = angle_kanan[i];
  }
}

void goto_servo_home_belok(float servo_c, float servo_f, float servo_t){
  float pwmrc1, realsudutrc1;
  float pwmrf1, realsudutrf1; // Kanan depan
  float pwmrt1, realsudutrt1;
